...
credit_data = credit_data[~(credit_data["Transaction Date"] <= "2020-10-1")]
credit_data = credit_data[~(credit_data["Transaction Date"] >= "2020-10-31")]

checking_data = checking_data[~(checking_data["Transaction Date"] <= "2020-10-1")]
checking_data = checking_data[~(checking_data["Transaction Date"] >= "2020-10-31")]

savings_data = savings_data[~(savings_data["Transaction Date"] <= "2020-10-1")]
savings_data = savings_data[~(savings_data["Transaction Date"] >= "2020-10-31")]
...
print(f"On October 2020 you made {netTotal} CAD")
